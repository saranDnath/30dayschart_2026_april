...
def main():
    ...
    data = df.iloc[5:].copy()
    data = data[[0, 11, 10]]
    data.columns = ['Country', 'Skilled_Birth', 'Maternal_Mortality']
    data['Skilled_Birth'] = pd.to_numeric(data['Skilled_Birth'], errors='coerce')
    data['Maternal_Mortality'] = pd.to_numeric(data['Maternal_Mortality'], errors='coerce')
    data = data.dropna()
    data['ShortName'] = data['Country'].apply(get_shortname)
    data['Highlight'] = data['ShortName'].isin(highlight_countries)
    data['Label'] = data.apply(
        lambda r: f"{r['ShortName']} ({r['Skilled_Birth']:.1f}, {r['Maternal_Mortality']:.1f})"
    ...
    ax.scatter(bg['Skilled_Birth'], bg['Maternal_Mortality'], color=PALETTE['dot_bg'], s=45, alpha=0.55, edgecolor='white', linewidths=0.4, zorder=1)

    hl = data[data['Highlight'] & (data['ShortName'] != 'Bangladesh')]
    ax.scatter(hl['Skilled_Birth'], hl['Maternal_Mortality'], color=PALETTE['dot_highlight'], s=180, alpha=0.92, edgecolor='white', linewidths=1.5, zorder=3)

    bd = data[data['ShortName'] == 'Bangladesh']
    if not bd.empty:
        ax.scatter(bd['Skilled_Birth'], bd['Maternal_Mortality'], color=PALETTE['dot_bd'], s=520, alpha=0.20, edgecolor='none', zorder=4)
        ax.scatter(bd['Skilled_Birth'], bd['Maternal_Mortality'], color=PALETTE['dot_bd'], s=260, edgecolor='white', linewidths=2.0, zorder=5)

    for _, row in data[data['Highlight']].iterrows():
        is_bd = row['ShortName'] == 'Bangladesh'
        dx, dy = (8, 6)
        ax.annotate(
            row['Label'], xy=(row['Skilled_Birth'], row['Maternal_Mortality']), xytext=(dx, dy), textcoords='offset points',
    ...
    ax.set_xlabel('Births Attended by Skilled Personnel (%)', fontsize=13, fontweight='bold', color=PALETTE['axis'], labelpad=10)
    ax.set_ylabel('Maternal Mortality Ratio (log scale)', fontsize=13, fontweight='bold', color=PALETTE['axis'], labelpad=10)
    ax.set_yscale('log')

    fig.text(0.2, 0.975, 'Maternal Mortality vs Skilled Birth Attendance', fontsize=17, fontweight='bold', color=PALETTE['title'], va='top')
    fig.text(0.2, 0.95, 'WHO World Health Statistics 2022  ·  Proportion of births attended by skilled personnel vs MMR', fontsize=10, color=PALETTE['title'], va='top', alpha=0.75)
    ...
    output_filename = os.path.join(script_dir, 'day_3_python.png')
