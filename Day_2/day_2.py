...
def main():
    ...
    for _, row in data[data['Highlight']].iterrows():
        ...
        dx, dy = (8, 6)
        ax.annotate(
            row['Label'], xy=(row['Life_Exp'], row['Healthy_Life_Exp']), xytext=(dx, dy), textcoords='offset points',
    ...
    ax.axline((0, 0), slope=1, color=PALETTE['axis'], linestyle='--', alpha=0.5, zorder=0)
