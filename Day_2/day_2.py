...
def main():
    ...
    ax.annotate('*axes start at 50 & 45', 
            xy=(50, 45), xytext=(4, 4),
            textcoords='offset points',
            fontsize=8, color=PALETTE['axis'],
            alpha=0.6, style='italic', zorder=8)
    ...
    for _, row in data[data['Highlight']].iterrows():
        ...
        t = ax.text(
            row['Life_Exp'], row['Healthy_Life_Exp'],
            row['Label'],
    ...
    adjust_text(
        ...
        ax=ax,
        expand=(1.2, 1.5),
        arrowprops=dict(arrowstyle='-', color='#8899AA', lw=0.6, alpha=0.5),
    )
    ...
    # Annotate highest
    ax.scatter(highest['Life_Exp'], highest['Healthy_Life_Exp'],
           color='#E8A020', s=180, edgecolor='white', linewidths=1.5, zorder=6)
    ax.annotate(
    f"{highest['ShortName']} ({highest['Life_Exp']:.1f}, {highest['Healthy_Life_Exp']:.1f})\n↑ Highest Life Expectancy",
    xy=(highest['Life_Exp'], highest['Healthy_Life_Exp']),
    xytext=(-125, 10), textcoords='offset points',
    fontsize=9, color='#8B5E00', fontweight='bold',
    path_effects=[pe.withStroke(linewidth=2.5, foreground='white')],
    zorder=8
)

    # Annotate lowest
    ax.scatter(lowest['Life_Exp'], lowest['Healthy_Life_Exp'],
           color='#E8A020', s=180, edgecolor='white', linewidths=1.5, zorder=6)
    ax.annotate(
    f"{lowest['ShortName']} ({lowest['Life_Exp']:.1f}, {lowest['Healthy_Life_Exp']:.1f})\n↓ Lowest Life Expectancy",
    xy=(lowest['Life_Exp'], lowest['Healthy_Life_Exp']),
